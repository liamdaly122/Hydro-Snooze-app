...
| The app says switching off did not work, but the unit is off | The plug is slow. A Shelly takes the better part of a minute to report a change of draw, so anything that asks it within about thirty seconds gets the reading from before. The app now waits two minutes before it believes a failure. If it still reports one, believe it |
| The chart only goes back so far | Five years of samples and five years of events are kept, pruned on the hour. That is about 516 MB. Both limits live in `service.py` as `HISTORY` and `EVENTS_KEPT`, and they have to move together |
...
| Everything unreachable at once, and power saving is the suspect | Not three devices, one network. Check `iw dev wlan0 get power_save` on the Pi. Raspberry Pi OS ships it **on**, and the symptom is a machine that is fine all day under traffic, drops when it goes idle overnight, and never comes back. `install.sh` turns it off, and since 19 September it reads the setting back and prints it rather than assuming. Careful with the check itself: `ssh pi "iw ..."` answers `command not found` even when `iw` is installed, because it lives in `/usr/sbin` and a non-interactive ssh shell has no `/usr/sbin` on its PATH. Use the full path, or log in first |
| It went down overnight | First `journalctl --list-boots` on the Pi. A new boot means the machine itself rebooted; only one boot means the service restarted or the Wi-Fi dropped, which are different faults. Then `journalctl -k -b -1 -n 120` for what the kernel said before it went, and `vcgencmd get_throttled` for whether the supply sagged. `./scripts/diagnose.py` collects all of it |
| `journalctl --list-boots` shows only one boot | The journal is not being kept across reboots, so every crash destroys its own evidence. `Storage=persistent` and `/var/log/journal`, which `install.sh` has set since 19 September. Rerun it, or set it by hand |

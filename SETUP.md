...
| Everything unreachable at once: plug, blaster and probes | Not three devices, one network. Check `iw dev wlan0 get power_save` on the Pi. Raspberry Pi OS ships it **on**, and the symptom is a machine that is fine all day under traffic, drops when it goes idle overnight, and never comes back. `install.sh` turns it off, and since 19 September it installs `iw` first rather than silently skipping when it is missing |
